...
from .serializer import FavListSerial,WishListSerial,RecomSerial,ItemSerial
...
def fav(request):
    ...
    if request.user.is_authenticated:
        favcnt = Favlist.objects.count()
        wishcnt = Wishlist.objects.count()
        user_fav_items = Favlist.objects.filter(user_id=int(request.user.id)).all()
        itm_ids = [i.item.item_no for i in user_fav_items]
        prod_list = Item.objects.filter(item_no__in=itm_ids)
        serial = ItemSerial(prod_list, many=True)
        print(serial.data)
        return Response({"itm": serial.data, "favcnt": favcnt, "wishcnt": wishcnt}, template_name='fav.html')
    return render(request, "fav.html", {"itm": serial, "favcnt": favcnt, "wishcnt": wishcnt})
