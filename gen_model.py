import sys
sys.path.append(r"C:\Users\Root\miniconda3\envs\tf_env\Lib\site-packages")
import os
import numpy as np
import pandas as pd
import random
...
if getT:
    try:
        connection = sqlite3.connect('db.sqlite3')  # SQLite database file
...
def generate_dataset():
    ...
    tfidf_matrix = vectorizer.fit_transform([t.lower() for t in title])
    ...
    for i in range(len(title)):
        for j in range(i + 1, len(title)):
            ...
            vec1 = tfidf_matrix[i].toarray()[0] * weight1
            vec2 = tfidf_matrix[j].toarray()[0] * weight2
            cosine_score = cosine_similarity([vec1], [vec2])[0][0]

            data['text1'].append(sentence1)
            data['text2'].append(sentence2)
            data['similarity_score'].append(round(cosine_score, 3))
